...
$$\text{Recall@10} = \frac{\text{# relevant assessments in top 10}}{\text{# total relevant assessments for query}}$$
...
|-------|-----------|-----|
| Turn cap not enforced | `end_of_conversation` flag computed but never checked | Added `total_turns >= MAX_TURNS` guard; set flag at turn 8 |
| Weak personality diversity | Top-scoring recs dominated by knowledge tests | Two-pass ranking: top 10 unique, then backfill missing types |
| GSA alias false signal | "GSA" acronym appeared in both Global Skills Assessment & Verify - General Ability Screen | Hardcoded special case: discard GSA from latter, boost former |
| Stakeholder preference mistaken for signal | Initial hint set conflated stakeholder communication with personality testing | Separated STAKEHOLDER_HINTS (domain-specific) from PERSONALITY_HINTS |
| Cold-start latency | First /health call on Render free tier slow | Documented 30s warm-up expectation; evaluator capped at 2 minutes |

## 4. Technical Stack & Trade-offs

| Component | Choice | Rationale |
|-----------|--------|-----------|
| **Framework** | FastAPI 0.136.1 | Fast, native async, excellent Pydantic integration for schema validation |
| **Server** | Uvicorn + Render (free tier) | Lightweight ASGI, free deployment with auto-redeploy on GitHub push |
| **Scraping** | BeautifulSoup4 + Requests | No JavaScript needed for SHL catalog; simple HTML parsing |
| **Schema Validation** | Pydantic 2.12.5 | Strict type enforcement; breaks early on malformed responses |
| **Retrieval** | Deterministic scoring (no embeddings) | Eliminates hallucination risk; reproducible & auditable; no vector DB needed |
| **LLM** | None in scoring; only in agent reply generation | Trade-off: no semantic search, but 100% catalog-grounded recommendations |
| **State Management** | Stateless (full history in request) | Simplifies deployment, no session DB, fits 8-turn constraint |
...
- **GitHub**: Public repo at https://github.com/PragathiKC444/SHL-Research-Intern (6 commits, latest: eceaae4 - "Remove tracked Python cache artifacts")
